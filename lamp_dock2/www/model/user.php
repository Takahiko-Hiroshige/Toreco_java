...
function get_user($db, $mail){
  ...
  return fetch_query($db, $sql, array($mail));
}
...
function login_as($db, $mail, $password){
  ...
  set_session_user_info('mail_address', 'user_name', $user['mail_address'], $user['user_name']);
  return $user;
}
...
function regist_user($db,$real_name,$user_name,$mail, $password,$password_confirmation, $question_code,$question_answer,$address,$user_introduction) {
  if(is_valid_user($db,$real_name, $user_name,$mail, $password, $password_confirmation,$question_code, $question_answer,$address,$user_introduction) === false){
    return false;
  }
  //falseなら処理がここで終了
  //falseでなければ下の処理で入力値が登録される処理に進む
  return insert_user($db, $real_name,$mail, $password,$question_code,$question_answer,$address,$user_introduction);
}
...
function is_valid_user($real_name,$user_name,$mail, $password,$password_confirmation,$question_code,$question_answer,$address,$user_introduction){
  // 短絡評価を避けるため一旦代入。
...
function insert_user($db, $real_name,$user_name,$mail, $password,$question_code,$question_answer,$address,$user_introduction){
  $sql = "
    INSERT INTO
  ...
  return execute_query($db, $sql, array($mail, $password));
